cluster_glm_two_sided_critical <- function(confidence_level, degrees_freedom) {
  stats::qt(1 - (1 - as.numeric(confidence_level)) / 2, df = degrees_freedom)
}
...
run_cluster_glm <- function() {
  ...
  coefficient_rows <- lapply(seq_along(coefficients), function(index) {
    ...
    critical <- cluster_glm_two_sided_critical(spec$confidenceLevel, df_value)
    margin <- critical * standard_errors[[index]]
