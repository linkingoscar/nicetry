...
run_imputation <- function() {
  ...
  if (length(missing_columns)) stop(paste0("MI_COLUMN_NOT_FOUND: ", paste(missing_columns, collapse = ", ")))
  frame <- data[, selected, drop = FALSE]
  researchpath_budget_mice(nrow(frame), ncol(frame), as.integer(spec$imputations), as.integer(spec$iterations))
  ...
  imputed <- tryCatch(
    mice::mice(frame, m = as.integer(spec$imputations), maxit = as.integer(spec$iterations), method = methods, predictorMatrix = predictor_matrix, seed = researchpath_seed(spec$seed), printFlag = FALSE),
    error = function(error) stop(paste0("MI_ESTIMATION_FAILED: ", conditionMessage(error)), call. = FALSE)
  )
  ...
  for (index in seq_len(as.integer(spec$imputations))) {
    check_cancel()
    write_progress("multiple_imputation", index / as.integer(spec$imputations), index, as.integer(spec$imputations))
  ...
  if (identical(spec$pooling, "rubin")) {
    ...
    pooled_estimates <- lapply(term_names, function(term) {
      ...
      pooled <- pool_rubin_estimates(
        q,
        u,
        vapply(fits, stats::df.residual, numeric(1)),
      ...
      list(
        ...
        confidenceLevel = pooled$confidenceLevel,
    ...
    pooled_analysis <- list(model = spec$pooledAnalysis, substantiveModelHash = spec$substantiveModelHash, m = as.integer(spec$imputations), method = "Rubin_Barnard_Rubin", confidenceLevel = researchpath_validate_confidence_level(spec$confidenceLevel), estimates = pooled_estimates)
  }
  ...
  list(
    ...
    familyResult = list(family = family, method = spec$method, imputations = as.integer(spec$imputations), iterations = as.integer(spec$iterations), failureChains = 0L, convergence = convergence, missingInformation = missing_information, artifacts = artifacts, poolingStatus = pooling_status, confidenceLevel = researchpath_validate_confidence_level(spec$confidenceLevel), pooledAnalysis = pooled_analysis, rubin = if (identical(spec$pooling, "rubin")) list(method = "Rubin_Barnard_Rubin", m = as.integer(spec$imputations), degreesOfFreedomMethod = "Barnard-Rubin", confidenceLevel = researchpath_validate_confidence_level(spec$confidenceLevel), estimates = pooled_estimates) else NULL, trace = trace_diagnostics, distribution = distribution_diagnostics, fractionMissingInformation = lapply(pooled_estimates, function(row) list(term = row$term, FMI = row$FMI, RIV = row$RIV)))
  )
}
