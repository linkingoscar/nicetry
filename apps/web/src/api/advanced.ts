...
export function runAdvancedAnalysis(
  ...
  const body = {
    ...
    spec,
  } satisfies AdvancedAnalysisRequest
...
export function getAdvancedAnalysisStatus(runId: string, signal?: AbortSignal): Promise<AdvancedJobResponse> {
  return requestJson(`/api/v1/advanced-analyses/${runId}`, signal ? { signal } : undefined)
}

export function getAdvancedAnalysisResult(runId: string, signal?: AbortSignal): Promise<AdvancedResultResponse> {
  return requestJson(`/api/v1/advanced-analyses/${runId}/result`, signal ? { signal } : undefined)
}
