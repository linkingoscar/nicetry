...
describe('AdvancedResultView', () => {
  ...
  it('renders correctly with sample flow, estimates, and warnings', () => {
    ...
    // Checks warnings
    expect(screen.getByText('方法警告（1）')).toBeInTheDocument()
    ...
    expect(screen.getByRole('table', { name: '估计边际均值' })).toBeInTheDocument()
    expect(screen.getByRole('table', { name: '事后对比' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '导出论文复现包' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '导出复现包（含数据）' })).toBeInTheDocument()
  })
