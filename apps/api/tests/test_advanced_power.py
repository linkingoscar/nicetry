...
def test_monte_carlo_invalid_dgp_fails_before_presenting_a_power_estimate() -> None:
    settings = get_settings()
    root = settings.project_root
    fixture = root / "apps/api/tests/fixtures/advanced/power/regression-monte-carlo.json"
    runner = root / "engine/R/run_advanced_analysis.R"
    spec = json.loads(fixture.read_text(encoding="utf-8"))
    spec["monteCarloParameters"]["dataGeneration"]["predictorCorrelation"] = 1.0
    with tempfile.TemporaryDirectory() as temporary:
        work = Path(temporary)
        input_path = work / "input.json"
        output_path = work / "output.json"
        ...
        environment["R_LIBS_USER"] = str(settings.r_library_path)
        completed = subprocess.run(
            [str(settings.rscript_path), "--vanilla", str(runner), str(input_path), str(output_path)],
            cwd=root,
            env=environment,
            capture_output=True,
            text=True,
    ...
    assert "POWER_MONTE_CARLO_TOO_MANY_FAILURES" in completed.stderr
