...
The full quality gate recalculates these references into a temporary directory through
`scripts/check-r-numeric-baselines.py`. It rejects package-version, result-shape and
numeric drift at the golden's declared tolerance, while leaving the tracked fixtures
untouched. An R package upgrade therefore requires an explicit review of both the
independent reference and the product-runner output before a golden is regenerated.

The analytic-power fixtures in the sibling `goldens/` directory are separately executed
through the product R runner in `test_advanced_power_goldens.py`. Keeping that check
separate prevents an independent-reference generator from accidentally masking a
request-to-runner regression.
