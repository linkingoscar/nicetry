...
fixture_output_dir <- Sys.getenv("RESEARCHPATH_REFERENCE_FIXTURE_OUTPUT_DIR", unset = reference_dir)
golden_dir <- Sys.getenv("RESEARCHPATH_GOLDEN_OUTPUT_DIR", unset = file.path(advanced_dir, "goldens"))
...
write_fixture <- function(value, name) {
  path <- file.path(fixture_output_dir, name)
