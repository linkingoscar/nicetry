...
from app.capability_catalog import (
    capability_family_gate_metadata,
    capability_gate_metadata,
)
...
class AdvancedCapabilitySlice:
    ...
    def to_dict(self) -> JsonObject:
        ...
        gate = capability_gate_metadata(self.id)
        value.update(gate.to_dict())
        return value
...
class AdvancedCapability:
    ...
    def to_dict(self) -> JsonObject:
        ...
        value["slices"] = [item.to_dict() for item in self.slices]
        gate = capability_family_gate_metadata(
            tuple(item.id for item in self.slices if item.execution_available)
        )
        value.update(gate.to_dict())
        return value
...
class AdvancedAnalysisRegistry:
    ...
    def capabilities(self) -> list[JsonObject]:
        ...
        return [
            ...
            for family in product_order
            if family in self._runners
            for capability in (self._capabilities[family],)
        ]
    ...
    def validate(self, spec: AdvancedAnalysisSpec) -> JsonObject:
        capability = self._capabilities[spec.family]
        capability_slice = self.slice_for_spec(spec)
        gate = capability_gate_metadata(
            capability_slice.id if capability_slice else f"{spec.family}.unclassified"
        )
        ...
        return {
            ...
            "validationLevel": gate.validation_level,
            "maturityLevel": gate.maturity_level,
            "publicationEligibility": gate.publication_eligibility,
            "publicationEligibilityReason": gate.publication_eligibility_reason,
            "validationEvidence": gate.validation_evidence.to_dict(),
    ...
    AdvancedCapability(
        ...
        slices=(
            AdvancedCapabilitySlice(
                ...
                "experimental",
                True,
                "显式量表题项、量表范围、均值/求和计分与 cluster；返回未裁剪聚合证据，不使用固定阈值自动裁决",
            ),
            AdvancedCapabilitySlice(
    ...
    AdvancedCapability(
        ...
        slices=(
            ...
            AdvancedCapabilitySlice(
                ...
                "experimental",
                True,
                "支持连续/WLSMV 有序 Bifactor、带可审计 Target 的连续 ESEM，以及 MML 二元 2PL/多分类 GRM IRT 与 DIF；有序 ESEM 尚未开放",
            ),
            AdvancedCapabilitySlice(
