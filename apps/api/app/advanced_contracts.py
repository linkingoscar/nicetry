...
class CommonAdvancedSpec(ContractModel):
    ...
    sample_version_id: str | None = Field(default=None, pattern=r"^[A-Za-z][A-Za-z0-9_-]{2,127}$")
    sample_hash: str | None = Field(default=None, pattern=r"^[a-f0-9]{64}$")
    structure_version_id: str | None = Field(default=None, pattern=r"^[A-Za-z][A-Za-z0-9_-]{2,127}$")
    structure_hash: str | None = Field(default=None, pattern=r"^[a-f0-9]{64}$")
    measurement_version_id: str | None = Field(default=None, pattern=r"^[A-Za-z][A-Za-z0-9_-]{2,127}$")
    measurement_hash: str | None = Field(default=None, pattern=r"^[a-f0-9]{64}$")
...
class ExperimentalDesignSpec(CommonAdvancedSpec):
    ...
    def validate_experiment_slice_1(self) -> "ExperimentalDesignSpec":
        ...
        if self.planned_contrasts:
            if self.post_hoc_adjustment not in {"holm", "benjamini_hochberg"}:
                raise ValueError("PLANNED_CONTRAST_ADJUSTMENT_NOT_SUPPORTED")
            if len(self.between_factors) != 1 or self.within_factors or self.covariate_ids:
...
class MultipleImputationSpec(CommonAdvancedSpec):
    ...
    plan_version_id: str | None = Field(default=None, pattern=r"^[A-Za-z][A-Za-z0-9_-]{2,127}$")
    imputation_plan_version_id: str | None = Field(default=None, pattern=r"^[A-Za-z][A-Za-z0-9_-]{2,127}$")
    predictor_matrix_hash: str | None = Field(default=None, pattern=r"^[a-f0-9]{64}$")
    ...
    substantive_model_hash: str | None = Field(default=None, pattern=r"^[a-f0-9]{64}$")
    diagnostics: list[
    ...
    def validate_imputation(self) -> "MultipleImputationSpec":
        ...
        if self.pooled_analysis is not None:
            model_payload = self.pooled_analysis.model_dump(by_alias=True)
            ...
            if (
                self.substantive_model_hash is not None
                and self.substantive_model_hash != computed_hash
            ):
                raise ValueError("MI_SUBSTANTIVE_MODEL_CHANGED_REIMPUTE_REQUIRED")
            self.substantive_model_hash = computed_hash
            analysis_ids = {
                self.pooled_analysis.outcome_id,
                *self.pooled_analysis.predictor_ids,
            }
            imputation_model_ids = set(variable_ids)
            for variable in self.variables:
                imputation_model_ids.update(variable.predictor_ids)
            omitted = sorted(analysis_ids - imputation_model_ids)
            if omitted:
                raise ValueError(
                    "MI_SUBSTANTIVE_MODEL_VARIABLES_REQUIRED: " + ", ".join(omitted)
                )
            for variable in self.variables:
                required_predictors = analysis_ids - {variable.variable_id}
                missing_predictors = sorted(
                    required_predictors - set(variable.predictor_ids)
                )
                if missing_predictors:
                    raise ValueError(
                        "MI_IMPUTATION_MODEL_INCOMPATIBLE: "
                        f"{variable.variable_id} 缺少 "
                        + ", ".join(missing_predictors)
                    )
        elif self.substantive_model_hash is not None:
            raise ValueError("MI_SUBSTANTIVE_MODEL_HASH_REQUIRES_ANALYSIS")
...
class MonteCarloParameters(ContractModel):
    data_generation: dict[str, object] = Field(description="生成数据的核心参数")
    estimand_target: str = Field(description="需要检验功效的效应项")
    missing_mechanism: dict[str, object] | None = None
    cluster_allocation: dict[str, object] | None = None
    repeated_measures_cov: dict[str, object] | None = None
...
class PowerAnalysisSpec(CommonAdvancedSpec):
    ...
    solve_for: Literal["sample_size", "power", "effect_size", "sensitivity", "ci_width"] = (
        "sample_size"
    )
    target_ci_width: float | None = Field(default=None, gt=0, alias="targetCIWidth")
    ...
    def validate_power(self) -> "PowerAnalysisSpec":
        ...
        if self.effect_size_metric is not None and self.effect_size_metric not in allowed:
            raise ValueError("POWER_EFFECT_METRIC_NOT_SUPPORTED")
        if self.solve_for == "ci_width":
            if self.method == "monte_carlo":
                raise ValueError("POWER_CI_WIDTH_ANALYTIC_ONLY")
            if self.target_ci_width is None:
                raise ValueError("POWER_TARGET_CI_WIDTH_REQUIRED")
            if self.sample_size is not None:
                raise ValueError("POWER_SAMPLE_SIZE_NOT_APPLICABLE")
        elif self.solve_for in {"effect_size", "sensitivity"}:
        ...
        if self.solve_for in {"power", "effect_size", "sensitivity"} and self.sample_size is None:
...
class QuestionnaireMeasurementSpec(CommonAdvancedSpec):
    ...
    def validate_measurement(self) -> "QuestionnaireMeasurementSpec":
        ...
        if self.model_type == "irt":
            if self.item_scale != "ordinal":
                raise ValueError("IRT_ITEMS_MUST_BE_DECLARED_ORDINAL")
            if self.estimator != "MML":
                raise ValueError("IRT_REQUIRES_MML_ESTIMATOR")
        elif self.estimator == "MML":
            raise ValueError("MML_ESTIMATOR_IS_ONLY_VALID_FOR_IRT")
        elif self.item_scale == "ordinal" and self.estimator != "WLSMV":
            raise ValueError("有序题项必须使用 WLSMV 估计器")
        if self.model_type != "irt" and self.item_scale == "continuous" and self.estimator == "WLSMV":
            raise ValueError("连续题项不得使用 WLSMV 估计器")
        if self.model_type in {"esem", "esem_bifactor_irt"}:
            if self.rotation != "target":
                raise ValueError("ESEM_REQUIRES_TARGET_ROTATION")
            if self.factor_count != len(self.constructs):
                raise ValueError("ESEM_TARGET_FACTOR_COUNT_MUST_MATCH_CONSTRUCT_COUNT")
            if self.item_scale != "continuous":
                raise ValueError("ORDINAL_ESEM_WLSMV_NOT_IMPLEMENTED")
...
class AdvancedAnalysisRequest(ContractModel):
    ...
    draft_id: str | None = Field(default=None, pattern=r"^draft_[a-f0-9]{32}$")
