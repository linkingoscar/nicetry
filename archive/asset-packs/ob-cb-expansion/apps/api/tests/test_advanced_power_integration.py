...
def test_power_monte_carlo_runner_returns_a_schema_valid_result() -> None:
    spec = {
        **_common("power_analysis", "analysis_power_monte_carlo"),
        "designFamily": "regression",
        "method": "monte_carlo",
        "solveFor": "power",
        "alpha": 0.05,
        ...
        "monteCarloParameters": {
            "dataGeneration": {"errorSd": 1.0, "predictorCorrelation": 0.0},
            "estimandTarget": "overall",
    ...
    result = result_response.json()
    validate_contract(result, get_settings().advanced_result_schema_path)
    assert result["familyResult"]["method"] == "monte_carlo"
    assert result["familyResult"]["validSimulations"] == 1000
