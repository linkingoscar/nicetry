...
from app.services import advanced_runner
...
def _run(
    ...
    monkeypatch.setattr(advanced_runner.subprocess, "Popen", popen)
    monkeypatch.setattr(advanced_runner, "validate_contract", lambda *_args: None)
...
def test_runner_cancels_and_times_out_by_terminating_the_process_tree(
    ...
    monotonic = iter([0.0, advanced_runner.ADVANCED_TIMEOUT_SECONDS + 1.0])
    monkeypatch.setattr(advanced_runner.time, "monotonic", lambda: next(monotonic))
...
def test_runner_forwards_new_progress_updates(
    ...
    monkeypatch.setattr(advanced_runner.subprocess, "Popen", popen)
    monkeypatch.setattr(advanced_runner, "validate_contract", lambda *_args: None)
    monkeypatch.setattr(advanced_runner.time, "sleep", lambda _seconds: None)
