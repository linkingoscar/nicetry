...
export function JobProgress({ jobId, initialJob, capability, onComplete, onCancel }: JobProgressProps) {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        const currentJob = await getAdvancedAnalysisStatus(jobId)
        if (!mounted) return
        ...
        if (currentJob.status === 'succeeded') {
          try {
            const res = await getAdvancedAnalysisResult(jobId)
  ...
  return (
    <section className="adv-progress-panel" aria-label="分析进度">
      ...
      <div className="adv-progress-body">
        ...
        {/* Stage info */}
        <div className="adv-progress-stage" aria-live="polite" aria-atomic="true">
