run_aggregation <- function() {
  ...
  evidence <- calc_multilevel_aggregation(analysis_data, ".rp_aggregation_score", ".rp_cluster", as.numeric(spec$scaleMin), as.numeric(spec$scaleMax))
  estimates <- list(
  ...
  list(
    sampleFlow = list(original = nrow(data), included = sum(complete.cases(analysis_data)), excluded = nrow(data) - sum(complete.cases(analysis_data)), missingMethod = "complete_cases", clusters = evidence$numClusters),
    estimates = estimates,
    diagnostics = list(message_entry("AGGREGATION_EVIDENCE_COMPLETED", "info", evidence$diagnosticMessage)),
    warnings = if (isTRUE(evidence$aggregationJustified)) list() else list(message_entry("AGGREGATION_NOT_JUSTIFIED", "warning", evidence$diagnosticMessage)),
