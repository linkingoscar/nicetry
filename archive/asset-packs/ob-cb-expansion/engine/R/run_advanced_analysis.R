...
if (identical(family, "questionnaire_measurement")) {
  ...
  source(file.path(script_dir, "lib", "parallel.R"), local = environment())
  source(file.path(script_dir, "lib", "resource_budget.R"), local = environment())
  source(file.path(script_dir, "lib", "validity.R"), local = environment())
  source(file.path(script_dir, "lib", "efa.R"), local = environment())
  source(file.path(script_dir, "lib", "cfa.R"), local = environment())
  source(file.path(script_dir, "lib", "cfa_validity.R"), local = environment())
  source(file.path(script_dir, "lib", "invariance.R"), local = environment())
  source(file.path(script_dir, "lib", "esem_bifactor.R"), local = environment())
  source(file.path(script_dir, "lib", "cmb.R"), local = environment())
  source(file.path(script_dir, "lib", "questionnaire_measurement_runner.R"), local = environment())
}
if (identical(family, "multiple_imputation")) {
  source(file.path(script_dir, "lib", "mi_rubin.R"), local = environment())
  source(file.path(script_dir, "lib", "imputation_runner.R"), local = environment())
}
if (identical(family, "multilevel_model")) {
  source(file.path(script_dir, "lib", "multilevel_aggregation.R"), local = environment())
  source(file.path(script_dir, "lib", "aggregation_runner.R"), local = environment())
}
if (identical(family, "longitudinal_model")) {
  source(file.path(script_dir, "lib", "longitudinal_advanced.R"), local = environment())
}
if (identical(family, "power_analysis")) {
  source(file.path(script_dir, "lib", "runtime.R"), local = environment())
  source(file.path(script_dir, "lib", "power_monte_carlo.R"), local = environment())
  source(file.path(script_dir, "lib", "power_t_test.R"), local = environment())
  source(file.path(script_dir, "lib", "power_analytic.R"), local = environment())
}
if (identical(family, "experimental_design")) {
  source(file.path(script_dir, "lib", "experimental_cluster_glm.R"), local = environment())
  source(file.path(script_dir, "lib", "experiment_posthoc.R"), local = environment())
}
...
run_experimental <- function() {
  ...
  if (length(factors)) {
    ...
    if (length(spec$plannedContrasts)) {
      ...
      planned_summary <- as.data.frame(summary(
        emmeans::contrast(planned_grid, method = planned_methods),
        infer = c(TRUE, TRUE),
      ...
      planned_summary$plannedContrastId <- names(planned_methods)
      planned_summary$multiplicityFamilyId <- vapply(
        names(planned_methods), function(id) planned_families[[id]], character(1)
      )
      planned_summary$analysisRole <- "planned_contrast"
      planned_summary$adjustment <- adjustment
  ...
  if (length(planned_contrast_rows)) {
    for (planned_row in planned_contrast_rows) {
      ...
      missing_fields <- names(report_values)[!vapply(report_values, is_finite_scalar, logical(1))]
      if (length(missing_fields) == 0L) {
        apaReports[[length(apaReports) + 1L]] <- sprintf("For planned contrast %s, estimate = %.3f, SE = %.3f, df = %.2f, p = %.3f, %d%% CI [%.3f, %.3f].", planned_row$plannedContrastId, planned_row$estimate, planned_row$SE, planned_row$df, planned_row$p.value, as.integer(as.numeric(spec$confidenceLevel) * 100), planned_row$lower.CL, planned_row$upper.CL)
      } else {
  ...
  list(
    ...
    familyResult = list(family = family, omnibusTests = omnibus, estimatedMarginalMeans = emm_rows, contrasts = contrast_rows, plannedContrasts = planned_contrast_rows, sphericity = sphericity),
...
run_longitudinal <- function() {
  ...
  if (!identical(spec$modelType, "growth_curve") && identical(spec$missing, "available_rows_ml")) stop("LONGITUDINAL_AVAILABLE_ROWS_ONLY_FOR_OBSERVED_GROWTH")
  ...
  list(
    ...
    warnings = if (length(stable_keys) > 1L && identical(spec$modelType, "growth_curve")) list(message_entry("GROWTH_FIRST_OUTCOME", "warning", "Growth curve execution used the first stable outcome key; run separate specifications for additional outcomes")) else list(),
...
write_json(result, args[[2]], auto_unbox = TRUE, pretty = TRUE, null = "null", digits = NA)
